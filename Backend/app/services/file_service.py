...
class FileService:
    ...
    def get_avatar_url(self, avatar_path: str, request: Optional[Request] = None) -> Optional[str]:
        """Возвращает URL для доступа к аватару"""
        if not avatar_path:
            return None
        # Используем request для получения базового URL
        base_url = str(request.base_url).rstrip('/')
        # Убираем лишний /static если он уже есть в base_url
        if base_url.endswith('/static'):
            base_url = base_url[:-7]
        
        # Возвращаем полный URL
        return f"{base_url}/static/{avatar_path}"
